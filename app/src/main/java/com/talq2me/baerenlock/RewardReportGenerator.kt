...
class RewardReportGenerator {
    ...
    /**
     * Generates a section of reward time app usage that can be appended to a daily report
     * This is used for appending multiple reward sessions throughout the day
     */
    fun generateReportSection(
        ...
        return buildString {
            appendLine("Rewards used for period: ${dateFormat.format(startDate)} - ${dateFormat.format(endDate)}")
            ...
            appendLine() // Extra line for spacing between sections
        }
    }
    
    /**
     * Generates a full text report of reward time app usage (for backward compatibility)
     */
    fun generateReport(
        sessions: List<RewardUsageTracker.AppUsageSession>,
        summary: RewardUsageTracker.RewardSessionSummary,
